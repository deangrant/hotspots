...
These snippets are illustrations, not crate-ready code. They omit crate docs
and `Debug`, and example 1 uses `eprintln!`. Pasting them into a member with
`[lints] workspace = true` will fail `[workspace.lints]` (`missing_docs`,
`missing_debug_implementations`, `print_stderr`).
